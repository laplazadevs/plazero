import type { MemeContestRow } from '../db/schemas.js';
import type { MemeContest, MemeData } from '../types/meme.js';
...
import { getCurrentFridayAtNoon, getNextFridayAtNoon } from '../utils/meme-utils.js';
...
export class MemeManager extends Context.Service<MemeManager>()('plazero/MemeManager', {
    make: Effect.gen(function* () {
        ...
        const addMemeToContest = Effect.fn('MemeManager.addMemeToContest')(function* (
            contestId: string,
            _memeData: MemeData
        ) {
            const contest = yield* getContest(contestId);
            ...
            // Only winners are stored; kept for compatibility with old callers.
            return true;
        });
        ...
        const getUserStats = Effect.fn('MemeManager.getUserStats')(function* (userId: string) {
            const stats = yield* memeRepo.getUserStats(userId);
            return {
        ...
        const cleanupOldContests = Effect.fn('MemeManager.cleanupOldContests')(function* (
            daysOld: number = 30
        ) {
            return yield* memeRepo.cleanupOldContests(daysOld);
        });
...
export const MemeManagerLive = Layer.effect(MemeManager)(MemeManager.make);
