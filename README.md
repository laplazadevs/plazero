...
- `/memeoftheyear` - Get the most reacted meme of the current year.
...
- **`SEND_MESSAGES`** - Post meme winners, voting messages, and responses.
- **`USE_APPLICATION_COMMANDS`** - Handle `/gettop`, `/memeoftheyear`, `/vote-timeout`, `/cancel-vote`.
- **`ADD_REACTIONS`** - Add 👍/👎 reactions to voting messages.
- **`READ_MESSAGE_HISTORY`** - Fetch messages for meme competitions.
- **`VIEW_CHANNEL`** - Access channels to read and send messages.
- **`EMBED_LINKS`** - Send rich embed messages for voting interface.

##### Moderation permissions
- **`MODERATE_MEMBERS`** - Apply timeouts based on voting results.
- **`MANAGE_MESSAGES`** - Edit voting messages with live updates.

##### Advanced permissions
- **`READ_MESSAGE_HISTORY && VIEW_CHANNEL`** - Access message content and reactions.
- **`USE_EXTERNAL_EMOJIS`** - Track custom server emojis for meme competitions.
...
- Weekly period: Friday 12:00 p.m. to the following Friday 12:00 p.m.
